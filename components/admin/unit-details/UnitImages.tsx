    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-3 items-center">
          {(unit.ImageUrls?.length || (unit as unknown as { imageUrls?: string[] }).imageUrls?.length || 0) > 0 && (
      ...
      {(!unit.ImageUrls || unit.ImageUrls.length === 0) && (!(unit as unknown as { imageUrls?: string[] }).imageUrls || ((unit as unknown as { imageUrls?: string[] }).imageUrls?.length ?? 0) === 0) ? (
        <label htmlFor="unit-detail-upload" className="block border-2 border-dashed border-gray-200 hover:border-brand-primary hover:bg-gray-50 transition-colors rounded-2xl p-10 text-center text-gray-400 cursor-pointer">
          <p className="text-sm">No images yet. Click here to upload.</p>
        </label>
      ) : (
        <div className="grid grid-cols-3 gap-3">
          {(unit.ImageUrls || (unit as unknown as { imageUrls?: string[] }).imageUrls || []).map((url: string, i: number) => {
