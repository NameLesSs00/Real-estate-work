      ...
      <div className="bg-white rounded-[24px] w-full max-w-[500px] flex flex-col shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-10 text-center space-y-6">
          <p className="text-[#475467] text-[18px] leading-relaxed">
            Are you sure you want to delete <span className="font-bold text-[#000000]">{projectName || 'this project'}</span>? This action cannot be undone.
          </p>
          {error && <p className="text-red-500 text-sm">{error}</p>}
          <div className="flex gap-4">
            <button onClick={onClose} className="flex-1 py-4 rounded-xl border border-gray-200 text-[#000000] font-bold hover:bg-gray-50 transition-colors cursor-pointer">Cancel</button>
