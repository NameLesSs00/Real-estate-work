...
export default function AdminFAQPage() {
  ...
  const inputCls = 'w-full bg-[#F8F5F0] border-none rounded-xl px-5 py-4 outline-none focus:ring-2 focus:ring-[#1B2134]/10 transition-all font-medium text-[#1B2134]';
  const inlineInputCls = 'w-full bg-white border-none rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-[#1B2134]/10 font-medium text-[#1B2134]';
    ...
    <div className="min-h-screen bg-[#FDFCFB] p-4 md:p-8 pt-10">
      <div className="max-w-[1100px] mx-auto">
        ...
        <div className="mb-8 flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-[32px] font-bold text-[#1B2134] font-radley mb-1">FAQ Management</h1>
          ...
          <button
            ...
            className="flex items-center gap-2 bg-[#1B2134] text-white px-6 py-3 rounded-full font-semibold text-[14px] shadow-md hover:scale-105 transition-all"
          >
          ...
          {showAdd && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-6">
              <div className="bg-white rounded-[24px] border border-[#F0EDE8] shadow-sm p-8">
                <h2 className="text-xl font-bold text-[#1B2134] mb-6 font-radley">New FAQ</h2>
                <form onSubmit={handleAdd} className="space-y-5">
                  <div className="space-y-2">
                    <label className="text-[13px] font-bold text-[#1B2134] ml-1">Question (Title)</label>
                  ...
                  <div className="space-y-2">
                    <label className="text-[13px] font-bold text-[#1B2134] ml-1">Answer (Description)</label>
                  ...
                  <button type="submit" disabled={saving} className="bg-[#1B2134] text-white px-10 py-4 rounded-full font-bold text-[15px] shadow-lg hover:scale-105 transition-all disabled:opacity-50 flex items-center gap-2">
        ...
        <div className="bg-white rounded-[32px] border border-[#F0EDE8] shadow-sm overflow-hidden">
          {loading ? (
            <div className="flex items-center justify-center p-16"><Loader2 className="animate-spin text-[#1B2134]" size={36} /></div>
          ) : questions.length === 0 ? (
            <div className="text-center p-16 text-[#1B2134]/40 font-medium">No FAQs yet. Click &quot;Add New FAQ&quot; to create the first one.</div>
          ) : (
            <>
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead>
                    <tr className="border-b border-[#F0EDE8] text-[#1B2134]/50 text-[12px] font-bold uppercase tracking-wider">
                  ...
                  <tbody className="divide-y divide-[#F0EDE8]">
                    {paginated.map((q, idx) => (
                      <React.Fragment key={q.id}>
                        <tr className="text-[#1B2134] hover:bg-[#FDFCFB] transition-colors">
                          <td className="px-6 py-5 text-[#1B2134]/40 font-semibold text-sm">{(page - 1) * PAGE_SIZE + idx + 1}</td>
                          ...
                          <td className="px-6 py-5">
                            <div className="flex items-center justify-end gap-2">
                              <button onClick={() => startEdit(q)} title="Edit" className="p-2 rounded-lg hover:bg-[#F0EDE8] text-[#1B2134] transition-colors"><Pencil size={16} /></button>
                            ...
                            <tr><td colSpan={4} className="px-6 pb-6">
                              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                                <div className="bg-[#F8F5F0] rounded-2xl p-6 mt-2">
                                  <h3 className="font-bold text-[#1B2134] mb-4 text-[15px]">Edit FAQ</h3>
                                  <form onSubmit={handleEdit} className="space-y-4">
                                    <div className="space-y-1"><label className="text-[12px] font-bold text-[#1B2134] ml-1">Question</label><input type="text" value={editForm.title} onChange={(e) => setEditForm({ ...editForm, title: e.target.value })} className={inlineInputCls} required /></div>
                                    <div className="space-y-1"><label className="text-[12px] font-bold text-[#1B2134] ml-1">Answer</label><textarea value={editForm.description} onChange={(e) => setEditForm({ ...editForm, description: e.target.value })} rows={3} className={`${inlineInputCls} resize-none`} required /></div>
                                    <div className="flex gap-3">
                                      <button type="submit" disabled={saving} className="bg-[#1B2134] text-white px-8 py-3 rounded-full font-bold text-[14px] hover:scale-105 transition-all disabled:opacity-50 flex items-center gap-2">{saving && <Loader2 className="animate-spin" size={16} />}Save Changes</button>
                                      <button type="button" onClick={() => setEditingId(null)} className="px-8 py-3 rounded-full font-bold text-[14px] border border-[#E0DBD4] text-[#1B2134] hover:bg-[#F0EDE8] transition-all">Cancel</button>
                                    </div>
                                  </form>
                                </div>
                              </motion.div>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-center gap-4 p-6 border-t border-[#F0EDE8]">
                  <button disabled={page === 1} onClick={() => setPage(page - 1)} className="p-2 rounded-full border border-[#F0EDE8] text-[#1B2134] disabled:opacity-30 hover:bg-[#F8F5F0] transition-colors"><ChevronLeft size={20} /></button>
                  <span className="font-semibold text-[15px] text-[#1B2134]">Page {page} of {totalPages}</span>
                  <button disabled={page === totalPages} onClick={() => setPage(page + 1)} className="p-2 rounded-full border border-[#F0EDE8] text-[#1B2134] disabled:opacity-30 hover:bg-[#F8F5F0] transition-colors"><ChevronRight size={20} /></button>
                </div>
              )}
            </>
          )}
