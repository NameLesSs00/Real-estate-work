...
export default function FacilitiesPage() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await createFacility({ name: newName, icon: newIcon });
  ...
  const handleUpdate = async (id: number) => {
    setLoading(true);
    try {
      await updateFacility({ id, name: editingName, icon: editingIcon });
  ...
  const handleEditClick = async (facility: Facility) => {
    ...
    setEditingIcon(facility.icon ?? null);
    
    // Fallback based on list data
    const nameObjRaw = typeof facility.name === 'object' ? facility.name : { en: facility.name, de: facility.name, it: facility.name };
    ...
    try {
      ...
      const [enRes, deRes, plRes] = await Promise.all([
        ...
        getFacilityById(facility.id, 'it')
      ]);
      ...
      setEditingIcon(enRes.icon ?? facility.icon ?? null);
    } catch (e) {
    ...
    <div className="p-8 md:p-12 min-h-screen font-inter bg-admin-bg">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div>
            <h1 className="text-[36px] font-bold text-brand-primary mb-2">Facilities</h1>
            <p className="text-admin-muted text-lg">Manage project and unit amenities.</p>
          </div>
          <button 
            ...
            >
              <form onSubmit={handleAdd} className="bg-white p-8 rounded-[32px] shadow-sm border border-gray-100 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-[14px] font-bold text-brand-primary ml-1">German Name</label>
                    <input 
                      type="text" 
                      value={newName.de}
                ...
                <div className="flex justify-end pt-2">
                  <button 
                    disabled={loading}
                    className="bg-brand-primary text-white px-10 py-4 rounded-xl font-bold shadow-md hover:scale-105 transition-all disabled:opacity-50 flex items-center gap-2"
                  >
                  ...
                  <td className="px-8 py-6">
                    {editingId === facility.id ? (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-[12px] font-semibold text-gray-400">{facility.icon ?? 'Default'}</span>
                      </div>
                    )}
                  </td>
                  <td className="px-8 py-6">
                    {editingId === facility.id ? (
                      ...
                      <span className="font-bold text-brand-primary text-[16px]">{getDisplayName(facility)}</span>
                    )}
                  </td>
