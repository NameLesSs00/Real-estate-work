...
export default function ApplicantsPage() {
  ...
  return (
    <div className="p-10 lg:p-14 font-inter bg-admin-bg min-h-full">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div>
          <h1 className="text-[36px] font-bold text-brand-primary mb-1">Applicants</h1>
          <p className="text-admin-muted text-[17px]">{totalCount} applicant{totalCount !== 1 ? 's' : ''} total</p>
        </div>
        <button
          onClick={() => {
          ...
          className="bg-brand-primary text-white px-8 py-4 rounded-full flex items-center gap-3 hover:bg-brand-primary-hover transition-all shadow-md cursor-pointer"
        >
          ...
          >
            <form onSubmit={editingId ? handleUpdate : handleAdd} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[14px] font-bold text-brand-primary">Full Name</label>
                <input 
                  type="text" 
                  value={formData.fullName}
                  onChange={(e) => setFormData({...formData, fullName: e.target.value})}
                  className="w-full bg-admin-bg border-none rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-brand-primary/20"
                  required
                />
              </div>
              <div className="space-y-2">
                <label className="text-[14px] font-bold text-brand-primary">Email</label>
                <input 
                  type="email" 
              ...
              <div className="space-y-2">
                <label className="text-[14px] font-bold text-brand-primary">Phone</label>
                <input 
                  type="text" 
              ...
              <div className="space-y-2">
                <label className="text-[14px] font-bold text-brand-primary">Notes</label>
                <input 
                  type="text" 
              ...
              <div className="md:col-span-2 flex justify-end gap-3 mt-2">
                ...
                <button type="submit" className="bg-brand-primary text-white px-8 py-3 rounded-xl font-bold shadow-md hover:bg-brand-primary-hover transition-colors">
      ...
      <div className="relative max-w-full mb-8">
        ...
        <input
          ...
          className="w-full bg-white border border-gray-100 rounded-[28px] py-5 pl-16 pr-10 text-[17px] text-brand-primary focus:outline-none focus:ring-4 focus:ring-brand-primary/5 transition-all shadow-sm placeholder:text-brand-muted-light"
          value={searchQuery}
      ...
      <div className="bg-white rounded-[32px] shadow-sm border border-gray-50 overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <button onClick={() => fetchApplicants(currentPage)} className="bg-brand-primary text-white px-6 py-2 rounded-full text-sm">Retry</button>
          </div>
          ...
          <div className="overflow-x-auto scrollbar-hide">
            <table className="w-full min-w-[800px] text-left border-collapse">
              <thead>
                <tr className="border-b border-gray-50 text-[14px] font-bold text-brand-primary">
              ...
              <tbody className="divide-y divide-gray-50">
                {filteredApplicants.map(applicant => (
                  <tr key={applicant.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="py-5 px-8 font-semibold text-brand-primary">{applicant.fullName}</td>
                    ...
                    <td className="py-5 px-8 text-right">
                      <div className="flex items-center justify-end gap-3">
                        <button onClick={() => startEdit(applicant)} className="p-2 text-gray-500 hover:text-brand-primary hover:bg-gray-100 rounded-lg transition-colors">
                          <Edit2 size={18} />
                        </button>
