    ...
    <div className="p-8 md:p-12 min-h-screen bg-brand-bg font-inter">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div>
            <h1 className="text-[36px] font-bold text-brand-primary mb-2">Property Leads</h1>
            <p className="text-admin-muted text-lg">Manage inquiries received for specific properties and projects.</p>
          </div>
          <div className="bg-white px-6 py-4 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
            ...
            <div className="w-12 h-12 bg-brand-secondary-soft rounded-xl flex items-center justify-center text-brand-primary">
              <Eye size={24} />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[32px] border border-gray-100 shadow-sm overflow-hidden">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 gap-4">
              ...
              <button onClick={() => fetchLeads(currentPage)} className="bg-brand-primary text-white px-8 py-3 rounded-xl font-bold cursor-pointer hover:bg-brand-primary-hover transition-all">Retry</button>
            </div>
          ) : filteredLeads.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-32 gap-4 text-gray-300">
              <Search size={48} />
              <p className="text-admin-muted text-lg font-medium">No leads found.</p>
            </div>
          ) : (
                  ...
                  {filteredLeads.map((lead) => (
                    <tr key={lead.id} className="hover:bg-gray-50/30 transition-colors">
                      <td className="px-8 py-6">
                        <div className="font-bold text-brand-primary">{lead.fullName}</div>
                        <div className="text-[13px] text-gray-400">{lead.email}</div>
                      </td>
                      <td className="px-8 py-6">
                        <div className="text-[14px] font-semibold text-brand-primary">{lead.propertyName || 'General Inquiry'}</div>
            ...
            <div className="flex items-center justify-between px-10 py-6 border-t border-gray-50 bg-gray-50/30">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="px-6 py-2.5 rounded-xl border border-gray-200 text-[14px] font-bold text-brand-primary bg-white hover:bg-gray-50 disabled:opacity-40 transition-all shadow-sm cursor-pointer"
                >Previous</button>
                <button
                  onClick={() => fetchLeads(currentPage + 1)}
              ...
              >
                ...
                <div className="p-10 space-y-6">
                  <div className="flex items-center gap-4 text-brand-primary">
                    <div className="w-12 h-12 bg-brand-secondary-soft rounded-2xl flex items-center justify-center shrink-0"><Mail size={20} /></div>
                  ...
                  <div className="flex items-center gap-4 text-brand-primary">
                    <div className="w-12 h-12 bg-brand-secondary-soft rounded-2xl flex items-center justify-center shrink-0"><Phone size={20} /></div>
                  ...
                  <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100">
                    ...
                    <p className="text-brand-primary font-bold text-lg">{selectedLead.propertyName || 'N/A'}</p>
                  ...
                  {selectedLead.notes && (
                    <div className="space-y-2">
                      ...
                      <p className="text-brand-primary leading-relaxed italic">&quot;{selectedLead.notes}&quot;</p>
                    </div>
                  )}
                  <button 
                    onClick={() => setSelectedLead(null)}
                    className="w-full bg-brand-primary text-white py-4 rounded-2xl font-bold mt-4 hover:bg-brand-primary-hover transition-all"
                  >Close</button>
                </div>
              </motion.div>
            </motion.div>
