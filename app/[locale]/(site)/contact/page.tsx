...
export default function ContactPage() {
  ...
  return (
    <main className="min-h-screen font-poppins overflow-hidden bg-[#fbf9f6]">
      ...
      <section className="relative z-20 -mt-24 px-6 pb-24">
        <div className="max-w-[1440px] mx-auto bg-white rounded-[32px] shadow-2xl overflow-hidden flex flex-col lg:flex-row">
          ...
          <div className="lg:w-2/5 bg-[#000000] text-white p-10 md:p-14 flex flex-col justify-between">
            <div>
              ...
              <div className="flex flex-col gap-10">
                {infoCards.map((card) => (
                  ...
                  >
                    <div className="w-12 h-12 bg-white/10 rounded-full flex items-center justify-center shrink-0 group-hover:bg-[#c7b7a1] transition-colors mt-1">
                    ...
                    <div className="flex-1">
                      ...
                      {card.socials ? (
                        <div className="flex flex-wrap gap-x-4 gap-y-2">
                          {card.socials.map((s) => (
                            <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" className="text-[15px] font-semibold text-white hover:text-[#c7b7a1] transition-colors block">{s.label}</a>
                          ))}
                        </div>
                      ) : (
                        <a href={card.href} target={card.href?.startsWith("http") ? "_blank" : undefined} rel="noopener noreferrer" className="text-[16px] font-bold text-white hover:text-[#c7b7a1] transition-colors block leading-relaxed break-words">
                          {card.content}
                        </a>
                      )}
                    </div>
          ...
          <div className="lg:w-3/5 p-10 md:p-14 bg-white">
            <h2 className="text-[32px] font-radley text-[#000000] mb-8">{t('contactPage.form.title') as string}</h2>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="flex flex-col gap-2">
                  ...
                  <input name="firstName" value={form.firstName} onChange={handleChange} type="text" placeholder={t('contactPage.form.placeholderFirstName') as string} required className="w-full border-b-2 border-gray-100 py-3 text-[15px] text-[#000000] outline-none focus:border-[#A88849] transition-colors placeholder:text-gray-300" />
                </div>
                <div className="flex flex-col gap-2">
                  <label className="text-[13px] font-semibold text-gray-500">{t('contactPage.form.lastName') as string} *</label>
                  <input name="lastName" value={form.lastName} onChange={handleChange} type="text" placeholder={t('contactPage.form.placeholderLastName') as string} required className="w-full border-b-2 border-gray-100 py-3 text-[15px] text-[#000000] outline-none focus:border-[#A88849] transition-colors placeholder:text-gray-300" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="flex flex-col gap-2">
                  ...
                  <input name="email" value={form.email} onChange={handleChange} type="email" placeholder={t('contactPage.form.placeholderEmail') as string} required className="w-full border-b-2 border-gray-100 py-3 text-[15px] text-[#000000] outline-none focus:border-[#A88849] transition-colors placeholder:text-gray-300" />
                </div>
                <div className="flex flex-col gap-2">
                  <label className="text-[13px] font-semibold text-gray-500">{t('contactPage.form.phone') as string} *</label>
                  <input name="phone" value={form.phone} onChange={handleChange} type="tel" placeholder={t('contactPage.form.placeholderPhone') as string} required className="w-full border-b-2 border-gray-100 py-3 text-[15px] text-[#000000] outline-none focus:border-[#A88849] transition-colors placeholder:text-gray-300" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="flex flex-col gap-2">
                  ...
                  <select name="inquiryType" value={form.inquiryType} onChange={handleChange} required className="w-full border-b-2 border-gray-100 py-3 text-[15px] text-[#000000] outline-none focus:border-[#A88849] transition-colors appearance-none cursor-pointer">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <select name="source" value={form.source} onChange={handleChange} required className="w-full border-b-2 border-gray-100 py-3 text-[15px] text-[#000000] outline-none focus:border-[#A88849] transition-colors appearance-none cursor-pointer">
              ...
              <div className="flex flex-col gap-2 mt-4">
                ...
                <textarea name="message" value={form.message} onChange={handleChange} placeholder={t('contactPage.form.placeholderMessage') as string} rows={4} required className="w-full border-b-2 border-gray-100 py-3 text-[15px] text-[#000000] outline-none focus:border-[#A88849] transition-colors resize-none placeholder:text-gray-300" />
              </div>

              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 mt-6">
                <label className="flex items-center gap-3 cursor-pointer">
                  <input name="agreed" type="checkbox" checked={form.agreed} onChange={handleChange} required className="w-5 h-5 rounded border-gray-300 accent-[#000000] cursor-pointer" />
                ...
                <motion.button 
                  ...
                  type="submit" 
                  disabled={loading} 
                  className="bg-[#000000] text-white px-10 py-4 rounded-full font-bold text-[15px] hover:bg-[#A88849] shadow-xl transition-all duration-300 disabled:opacity-60 disabled:cursor-not-allowed"
                >
